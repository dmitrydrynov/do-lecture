...
export class TelegramWidget {
	...
	auth(callback: (user: TelegramUserData) => void, setLoading: React.Dispatch<React.SetStateAction<boolean>>) {
		setLoading(true)
		...
		const popup_url =
			...
			(this.request_access ? '&request_access=' + encodeURIComponent(this.request_access) : '')
		const popup = window.open(popup_url, '_blank', `width=${width},height=${height},left=${left},top=${top},status=0,location=0,menubar=0,toolbar=0`)
	...
	private async getAuthData(callback: (userData: TelegramUserData) => void) {
		try {
			const response = await fetcherWithCredentials([this.widgetsOrigin + '/auth/get', { bot_id: this.bot_id }])
